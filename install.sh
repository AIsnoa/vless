...
checkCentosSELinux() {
    if [[ -f "/etc/selinux/config" ]] && ! grep -q "SELINUX=disabled" <"/etc/selinux/config"; then
        ...
        echoContent yellow "https://www.cxzyun.com/archives/1679931532764#heading-8 "
        exit 0
    fi
}
checkSystem() {
...
initTLSNginxConfig() {
    ...
    if [[ -n "${currentHost}" ]]; then
        ...
        if [[ "${historyDomainStatus}" == "y" ]]; then
            ...
            echo
            echoContent yellow "请输入要配置的域名 例: www.cxzyun.com --->"
            read -r -p "域名:" domain
        fi
    else
        echo
...
initDNSAPIConfig() {
    if [[ "$1" == "cloudflare" ]]; then
        echoContent yellow "\n CF_Token参考配置教程：https://www.cxzyun.com/archives/1701160377972\n"
...
manageCDN() {
    ...
    if echo "${currentInstallProtocolType}" | grep -qE ",1,|,2,|,3,|,5,"; then
        ...
        echoContent yellow "# 注意事项"
        echoContent yellow "\n教程地址:"
        echoContent skyBlue "https://www.cxzyun.com/archives/cloudflarezi-xuan-ip"
...
aliasInstall() {
    ...
    if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "初学者云官网：www.cxzyun.com
各种优质线路/全球稳定独享
原生住宅双isp家庭ip/多ip云服务器/多ip弹性服务器
多ip站群服务器/多ip宿主机/动态拨号VPS等各国各种服务器供你选择
可自己搭建sk5 ss vmess pptp L2tp open wireguard红龙等各种协议
不会搭建的-群文件有搭建脚本及视频教程-也可帮搭建好
另有偿教学搭建中转和多ip站群sk5, ss, vmess, http等协议
高质量独享ip，量大优惠，推广返现，招代理，详情进交流群:4267597
 请勿用于非法用途，否则后果与责任自行承担"; then
...
ipv6Routing() {
    ...
    if [[ "${ipv6Status}" == "1" ]]; then
        ...
        echoContent yellow "# 注意事项\n"
        echoContent yellow "# 注意事项"
        echoContent yellow "# 使用教程：https://www.cxzyun.com/archives/1683226921000 \n"
...
socks5Routing() {
    ...
    echoContent yellow "# 使用教程：https://www.cxzyun.com/archives/1683226921000#heading-5 \n"
...
setSocks5InboundRouting() {
    ...
    if [[ "${socks5InboundRoutingDomainStatus}" == "y" ]]; then
        ...
        echoContent yellow "录入示例:netflix,openai,cxzyun.com\n"
...
addOtherSubscribe() {
    echoContent yellow "#注意事项:"
    echoContent yellow "请仔细阅读以下文章： https://www.cxzyun.com/archives/1681804748677"
    echoContent skyBlue "录入示例：www.cxzyun.com:443:vps1\n"
...
initRealityDest() {
    if [[ -n "${domain}" ]]; then
        ...
        echoContent green "回落域名列表：https://www.cxzyun.com/archives/1680104902581#heading-8\n"
...
initRealityClientServersName() {
    ...
    if [[ -z "${realityServerName}" ]]; then
        ...
        echoContent skyBlue "\n================ 配置客户端可用的serverNames ===============\n"
        echoContent yellow "#注意事项"
        echoContent green "Reality目标可用域名列表：https://www.cxzyun.com/archives/1689439383686#heading-3\n"
...
manageReality() {
    ...
    if ! echo "${currentInstallProtocolType}" | grep -q -E "7,|8," || [[ -z "${coreInstallType}" ]]; then
        echoContent red "\n ---> 请先安装Reality协议，参考教程 https://www.cxzyun.com/archives/1680104902581#heading-11"
        exit 0
    fi
...
menu() {
    ...
    echoContent green "-初学者云官网：www.cxzyun.com"
    echoContent green "各种优质线路/全球稳定独享"
    echoContent green "原生住宅双isp家庭ip/多ip云服务器/多ip弹性服务器"
    echoContent green "多ip站群服务器/多ip宿主机/动态拨号VPS等各国各种服务器供你选择"
    echoContent green "可自己搭建sk5 ss vmess pptp L2tp open wireguard红龙等各种协议"
    echoContent green "不会搭建的-群文件有搭建脚本及视频教程-也可帮搭建好"
    echoContent green "另有偿教学搭建中转和多ip站群sk5, ss, vmess, http等协议"
    echoContent green "高质量独享ip，量大优惠，推广返现，招代理，详情进交流群:4267597"
    echoContent green "请勿用于非法用途，否则后果与责任自行承担"
    showInstallStatus
    checkWgetShowProgress
    echoContent red "\n==========================================================="
    echoContent red "                                              "
